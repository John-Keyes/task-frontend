...
const TaskCard = ({task, ...props}: TaskCardProps) => {
    ...
    return (
        <li {...props} className="fit-width task-card space-above" onClick={() => push(`/${task.id}`)}>
            <input id={`task-card-${task.id}`} onFocus={HandleOnFocus} onChange={HandleOnChange} type="checkbox" onBlur={HandleOnBlur} checked={task.completed} name={task.title as string} className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"/>
